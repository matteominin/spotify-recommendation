...
export async function playlistPrompt(playlists){
...
export async function songPrompt(songs, selectedSongs){
...
export async function popularityPrompt() {
    const res = await inquirer.prompt([{
        type: "number",
        name: "popularity",
...
export async function openBrowserPrompt(tracks) {
    await inquirer.prompt([{
        ...
        if(answer.open) 
            tracks.tracks.forEach(async elem => await open("https://open.spotify.com/track/" + elem.id))
    })
}